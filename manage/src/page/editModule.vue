    ...
    <div class="add-module">
      <div class="bread_box">
        <Breadcrumb>
          ...
          <BreadcrumbItem>编辑模块</BreadcrumbItem>
        </Breadcrumb>
      </div>
      <div class="content">
        <Form ref="moduleForm" :model="moduleForm" :rules="moduleRule" :label-width="80">
          <div class="module_com">
            ...
            <FormItem prop="moduleType" label="模块类型">
              <Select v-model="moduleForm.moduleType" disabled style="width:200px" placeholder="请选择">
        ...
        <div v-if="isModuleType">
          ...
          <div class="module_item" v-for="(item, index) in moduleItemNum">
            <p>第{{index+1}}列表项</p>
            <module-item :is-module-type="isModuleType" :module-data="moduleItemData[index]"></module-item>
          </div>
        </div>
        <div v-else>
          <div class="module_title">
          ...
          <module-item :is-module-type="isModuleType" :moduleData="moduleItemData[0]"></module-item>
        </div>
        <div class="submit_btn_box">
          <Button type="primary" @click="submitFormHandler">提交修改</Button>
        </div>
      </div>
    </div>
</template>
...
<script>
  ...
  import { editModule, getModuleDetail } from '../axios/getData'
  export default {
    name: "add-module",
    data(){
      return {
    ...
    methods: {
      init(){
        getModuleDetail(this.moduleId).then((response) => {
          if(!response.data.status){
            let res = response.data.data;
            this.moduleForm.name = res.name.text;
            this.moduleForm.sortIndex = res.sortIndex;
            this.moduleForm.moduleType = res.moduleType.toString();
            this.moduleItemData = res.content;
      ...
      submitFormHandler(){
        ...
        this.$refs.moduleForm.validate((valid) => {
          if(valid){
            editModule(this.moduleId, name, sortIndex, itemContent).then((response) => {
              if(!response.data.status){
